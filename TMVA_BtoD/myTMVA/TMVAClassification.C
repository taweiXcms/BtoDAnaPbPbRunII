...
int TMVAClassification(int pbpb=0, float ptMin=10., float ptMax=20., int varStage = 1, TString myMethodList = "")
{
   isPbPb = (bool)pbpb;
   ptmin = ptMin;
   ptmax = ptMax;
   ...
   TString outfileName(Form("ROOT/TMVA_%s_%.0f_%.0f_varStage%d.root",colsyst.Data(),ptmin,ptmax,varStage));
   TFile* outputFile = TFile::Open(outfileName,"RECREATE");
   ...
   if(varStage>=1){
      ...
      dataloader->AddVariable("DRestrk1Pt");//>
   ...
   if(varStage>=11){
     ...
     dataloader->AddVariable("DtktkRes_chi2cl");//>
   }
...
//                                        "nTrain_Signal=0:nTrain_Background=100000:nTest_Signal=0:nTest_Background=100000:SplitMode=Random:NormMode=NumEvents:!V" );
   ...
   if(varStage>=11){
     VarSet+=":VarProp[23]=FMax";
     VarSet+=":VarProp[24]=FMax";
   }
