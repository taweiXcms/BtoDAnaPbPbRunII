...
int weighPurePthat(TString ifname = "",
                   TString ofname = "")
{
  Bool_t isInsidebin(Float_t xpthat, Float_t maxDgenpt, Int_t i);
  cout<<endl;
  ...
  for(Int_t i=0;i<nentries;i++)
    {
      ...
      for(Int_t j=0;j<nBins;j++)
        {
          if(isInsidebin(pthat,maxDgenpt,j))
            {
...
Bool_t isInsidebin(Float_t xpthat, Float_t maxDgenpt, Int_t i)
{
  ...
  if(i<(nBins-1)
	&& (xpthat>pthatBin[i]&&maxDgenpt>ptBin[i])
    &&!(xpthat>pthatBin[i+1]&&maxDgenpt>ptBin[i+1])) return true;
  else if(i==(nBins-1)&&xpthat>pthatBin[i]&&maxDgenpt>ptBin[i]) return true;
  else return false;
}
